...
def verify_and_install():
    ...
    if missing_packages:
        if not is_admin():
            print("[!] Missing libraries found. Requesting Administrator privileges to install...")
            ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, f'"{sys.argv[0]}"', None, 1)
            sys.exit()

        for pip_name in missing_packages:
            print(f"[!] Administrator rights confirmed. Auto-installing '{pip_name}'...")
            try:
...
def speed_to_mbps(speed_value):
    if speed_value is None: return None
    if isinstance(speed_value, (int, float)): return int(speed_value)
...
def tool_slow_aps(make_pdf=True):
    ...
    if make_pdf:
        if data_rows:
            ...
            for r in data_rows: print(f"  - {r[0]} ({r[1]}) -> Switch: {r[2]}")
...
def tool_ap_statuses(status_filter, make_pdf=True):
    ...
    if make_pdf:
        ...
        if data_rows:
            ...
            for r in data_rows: print(f"  - {r[0]} (Status: {r[1]}) -> IP: {r[2]}")
...
def tool_existing_networks(make_pdf=True):
    ...
    if make_pdf:
        if data_rows:
            ...
            for r in data_rows[:10]: print(f"  - {r[0]} (ID: {r[1]})")
...
def tool_aps_down_today(make_pdf=True):
    ...
    if make_pdf:
        if data_rows:
            ...
            for r in data_rows: print(f"  - {r[0]} reported {r[2]} at {r[3]}")
...
def secure_exit(window, text_widget=None):
    ...
    try:
        del dashboard
        del ORG_ID
        if "MERAKI_API_KEY" in os.environ: del os.environ["MERAKI_API_KEY"]
        if "MFA_SECRET" in os.environ: del os.environ["MFA_SECRET"]
    except NameError:
        pass 
    gc.collect()
    window.destroy()
...
def process_mfa_flow(root_window):
    """Determines whether to show the MFA Setup screen or Verification screen."""
    root_window.destroy()
    load_dotenv()
    saved_secret = os.getenv("MFA_SECRET")

    if not saved_secret:
        # First time setup
        new_secret = pyotp.random_base32()
        mfa_setup_window(new_secret)
    else:
        # Existing user verification
        mfa_verify_window(saved_secret)

def verify_and_save_mfa(mfa_window, secret, user_code, is_setup=False):
    ...
    if totp.verify(user_code.strip()):
        if is_setup:
            ...
            if not dotenv_file:
                ...
                dotenv_file = find_dotenv()
            set_key(dotenv_file, "MFA_SECRET", secret)
            messagebox.showinfo("Success", "MFA successfully linked to your device!")
        
        # Access Granted!
        open_main_menu(mfa_window)
    else:
        messagebox.showerror("Denied", "Invalid Authenticator Code. Please try again.")

def mfa_setup_window(new_secret):
    """GUI window instructing the user how to link Google/MS Authenticator."""
    mfa_win = tk.Tk()
    mfa_win.title("MFA First-Time Setup")
    mfa_win.geometry("450x350")
    mfa_win.configure(bg="#2c3e50")
    
    mfa_win.update_idletasks()
    x = (mfa_win.winfo_screenwidth() // 2) - (450 // 2)
    y = (mfa_win.winfo_screenheight() // 2) - (350 // 2)
    mfa_win.geometry(f"+{x}+{y}")

    tk.Label(mfa_win, text="🛡️ Multi-Factor Authentication", font=("Arial", 16, "bold"), bg="#2c3e50", fg="white").pack(pady=15)
    
    instr = ("1. Open Google, Microsoft, or Duo Authenticator\n"
             "2. Select 'Add Account' -> 'Enter a setup key'\n"
             "3. Enter the exact key below:\n")
    tk.Label(mfa_win, text=instr, font=("Arial", 10), bg="#2c3e50", fg="#bdc3c7", justify="left").pack(pady=5)
    
    # Highlighted Secret Key
    tk.Label(mfa_win, text=new_secret, font=("Consolas", 18, "bold"), bg="#e74c3c", fg="white", padx=10, pady=5).pack(pady=5)

    tk.Label(mfa_win, text="4. Enter the generated 6-digit code below to verify:", font=("Arial", 10), bg="#2c3e50", fg="#bdc3c7").pack(pady=10)
    
    code_entry = tk.Entry(mfa_win, width=15, font=("Arial", 18), justify="center")
    code_entry.pack()

    btn = tk.Button(mfa_win, text="Verify & Save", font=("Arial", 10, "bold"), bg="#27ae60", fg="white", 
                    command=lambda: verify_and_save_mfa(mfa_win, new_secret, code_entry.get(), is_setup=True))
    btn.pack(pady=20)
    
    mfa_win.bind('<Return>', lambda event: verify_and_save_mfa(mfa_win, new_secret, code_entry.get(), is_setup=True))
    mfa_win.protocol("WM_DELETE_WINDOW", lambda: secure_exit(mfa_win))
    mfa_win.mainloop()

def mfa_verify_window(saved_secret):
    """GUI window asking for the daily 6-digit code."""
    mfa_win = tk.Tk()
    mfa_win.title("Security Verification")
    mfa_win.geometry("350x250")
    mfa_win.configure(bg="#2c3e50")

    mfa_win.update_idletasks()
    x = (mfa_win.winfo_screenwidth() // 2) - (350 // 2)
    y = (mfa_win.winfo_screenheight() // 2) - (250 // 2)
    mfa_win.geometry(f"+{x}+{y}")

    tk.Label(mfa_win, text="🛡️ 2-Step Verification", font=("Arial", 16, "bold"), bg="#2c3e50", fg="white").pack(pady=20)
    tk.Label(mfa_win, text="Open your Authenticator App\nand enter the 6-digit code:", font=("Arial", 10), bg="#2c3e50", fg="#bdc3c7").pack(pady=10)

    code_entry = tk.Entry(mfa_win, width=15, font=("Arial", 18), justify="center")
    code_entry.pack()
    code_entry.focus_set()

    btn = tk.Button(mfa_win, text="Unlock Dashboard", font=("Arial", 10, "bold"), bg="#2980b9", fg="white", 
                    command=lambda: verify_and_save_mfa(mfa_win, saved_secret, code_entry.get(), is_setup=False))
    btn.pack(pady=20)

    mfa_win.bind('<Return>', lambda event: verify_and_save_mfa(mfa_win, saved_secret, code_entry.get(), is_setup=False))
    mfa_win.protocol("WM_DELETE_WINDOW", lambda: secure_exit(mfa_win))
    mfa_win.mainloop()
...
def perform_manual_login(login_window, api_entry, org_entry):
    ...
    try:
        ...
        # If manual login succeeds, enforce MFA
        process_mfa_flow(login_window)
...
def create_splash_screen():
    ...
    def finish_splash(success):
        ...
        if success:
            ...
            # If auto-login succeeds, intercept it and process MFA!
            splash.after(1500, lambda: process_mfa_flow(splash))
        else:
